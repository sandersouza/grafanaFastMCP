...
@pytest.mark.parametrize(
    ...
    [
        ("https://grafana.local/d/uid", {"viewPanel": "1"}, "https://grafana.local/d/uid?viewPanel=1"),
        ("https://grafana.local/d/uid?viewPanel=1", {"foo": "bar"}, "https://grafana.local/d/uid?viewPanel=1&foo=bar"),
        ("https://grafana.local", {}, "https://grafana.local"),
    ],
)
def test_append_query_handles_existing_parameters(base: str, params: Dict[str, str], expected: str) -> None:
    assert navigation._append_query(base, params) == expected


def test_generate_deeplink_supports_multiple_resource_types(ctx: SimpleNamespace) -> None:
    app = FastMCP()
    navigation.register(app)

    tools = asyncio.run(app.list_tools())
    deeplink_tool = next(tool for tool in tools if tool.name == "generate_deeplink")

    result_dashboard = asyncio.run(
        deeplink_tool.function(resourceType="dashboard", dashboardUid="abc", ctx=ctx)
    )
    ...
    result_explore = asyncio.run(
        deeplink_tool.function(resourceType="explore", datasourceUid="loki", ctx=ctx)
    )
    ...
    with pytest.raises(ValueError):
        asyncio.run(
            deeplink_tool.function(resourceType="panel", dashboardUid="abc", ctx=ctx)
        )
    with pytest.raises(ValueError):
