...
def _filter_datasources(datasources: List[Dict[str, Any]], ds_type: Optional[str]) -> List[Dict[str, Any]]:
    if not ds_type:
        return datasources
    lowered = ds_type.lower()
    return [ds for ds in datasources if lowered in str(ds.get("type", "")).lower()]
...
async def _list_datasources(ctx: Context, ds_type: Optional[str]) -> List[Dict[str, Any]]:
    config = get_grafana_config(ctx)
    client = GrafanaClient(config)
    datasources = await client.get_json("/datasources")
    if not isinstance(datasources, list):
        raise ValueError("Unexpected response format from Grafana while listing datasources")
...
async def _get_by_name(ctx: Context, name: str) -> Any:
    ...
    try:
        ...
        if exc.status_code == 404:
            raise ValueError(f"Datasource with name '{name}' not found") from exc
        raise
...
def register(app: FastMCP) -> None:
    ...
    async def get_datasource_by_uid(
        ...
        if ctx is None:
            raise ValueError("Context injection failed for get_datasource_by_uid")
    ...
    async def get_datasource_by_name(
        ...
        if ctx is None:
            raise ValueError("Context injection failed for get_datasource_by_name")
