...
def _default_time_range(start: Optional[str], end: Optional[str]) -> tuple[datetime, datetime]:
...
async def _list_profile_types(
    ...
    if not isinstance(payload, dict):
        raise ValueError("Unexpected response from Pyroscope profile types API")
...
def register(app: FastMCP) -> None:
    ...
    async def fetch_pyroscope_profile(
        ...
        if ctx is None:
            raise ValueError("Context injection failed for fetch_pyroscope_profile")
