...
- [Configuration and security](./CONFIG-SECURITY.md)
- [Testing and quality](./TEST-QUALITY.md)
- [Worktree isolation](./WORKTREE-ISOLATION.md)
- [Memory and decisions](./MEMORY.md)
- [Agent onboarding guide](./ONBOARDING.md)
...
1. Read one row in [context packs](./CONTEXT-PACKS.md).
2. Use [semantic map](./SEMANTIC-MAP.md) to locate the owner and closest test.
3. Read only the owner file, closest test, and exact `rg` hits needed for the task.
4. Use deeper docs such as architecture, patterns, or tool modules only when blocked.
5. Run focused tests first, then `uv run ruff check .`.
...
- In `DESIGN`, do not load the full docs folder; stop after owner, test target, validation, and risks are clear.
- In `CODE_COMPLETION`, start from `git diff` and failing checks; do not restart architecture discovery.
